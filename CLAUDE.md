...
exceljs is NOT bundled: `ExcelExporter` lazy-loads it with `import('exceljs')` on the first export. Rollup marks `exceljs`, `react-virtuoso` and `@mui/icons-material` (regex) `external` — the consumer's bundler resolves and code-splits them from `dependencies`; inlining them would ship a second copy (e.g. a second `react-virtuoso` scroll observer) into apps that already use one. The download itself is a local `saveBlob` helper (object URL + synthetic `<a download>`), not a dependency; `ExcelExporter.test.ts` runs under happy-dom and intercepts `URL.createObjectURL` plus `HTMLAnchorElement.prototype.click` to assert on it.
