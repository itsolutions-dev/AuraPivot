...
- TypeScript strict everywhere in the library source (`tsconfig.json` has `"strict": true`). No `@ts-nocheck`/`@ts-ignore`/`@ts-expect-error` anywhere — do not add any. `rollup.config.js` and `scripts/verify-dist.mjs` are the only plain-JS files in the repo, both build tooling, not library source.
